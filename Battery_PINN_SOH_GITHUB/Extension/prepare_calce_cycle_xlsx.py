...
def parse_temp_and_condition_id(path: str):
    # Example filename contains "...-10DU 01.xlsx"
...
def make_global_cycle(df_part: pd.DataFrame, xlsx_path: str) -> pd.DataFrame:
    """
    Convert per-block cycles (often 1..50 in every DOE file) into a global cycle index.
    Example: DOE-051-100-... has lo=51, so we add offset=50 to cycle -> 51..100.
    """
    lo, hi = parse_cycle_range(xlsx_path)
    if lo is None:
        return df_part  # filename didn't have cycle range, leave unchanged
...
def extract_features_from_continuous(xlsx_path: str, q0: float, tempC: float, cond_id: int):
    ...
    # Use Step_Index to isolate discharge step more reliably
    # Strategy: find the Step_Index within this cycle that has the most negative total current (largest discharge)
        if STEP_COL in g.columns:
            step_groups = g.groupby(STEP_COL)
            step_score = step_groups[I_COL].sum()   # discharge step tends to have a large negative sum
            discharge_step = step_score.idxmin()    # most negative
    ...
    continue
        ...
        # ---- choose a robust window for "current features" ----
        # If CV window is missing/too small, fall back to CC window (still meaningful and avoids NaNs)
        ...
        # current stats (robust window: CV if possible else CC)
        ...
        # CC time and charge quantity (if CC exists; otherwise 0)
        cc_time = (np.nanmax(t_cc) - np.nanmin(t_cc)) if t_cc.size else 0.0
        cc_q = trapz_ah(t_cc, i_cc) if t_cc.size else 0.0

        # CV time and charge quantity (if CV exists; otherwise 0)
...
# robust: capacity is often cumulative; use (max - min)
        qd_cycle = float(np.nanmax(qd) - np.nanmin(qd))
        if not np.isfinite(qd_cycle) or qd_cycle <= 0:
            continue
...
# IMPORTANT: do NOT compute capacity here (block-local baseline causes inconsistency).
# We keep raw_Qd_Ah and compute capacity globally after concatenation.
    return out
...
def main():
    ...
    print(f"[OK] Found {sum(len(v) for v in groups.values())} continuous files in {len(groups)} (temp,cond) groups.")
        # ---- DEBUG: show what files are in each (temp, cond) group ----
...
# ---- compute capacity globally for this (temp,cond) group ----
# baseline = raw_Qd_Ah at the earliest global cycle
...
# drop raw capacity now that capacity is computed
