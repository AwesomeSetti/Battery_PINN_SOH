...
def load_data(args: argparse.Namespace, small_sample: Optional[int] = None):
    root = args.root
    data = CalceData(root=root, args=args)
    ...
    test_list = paths[:n_test]
    train_list = paths[n_test:]
    ...
    if len(test_list) > 0:
        ...
        print("NUM FILES train:", len(train_list), "test:", len(test_list))
...
def get_args():
    parser = argparse.ArgumentParser("Hyper Parameters for Calce dataset (processed CSVs)")
